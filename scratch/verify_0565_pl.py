# 확인 폴더의 0565 PL 파일 기준으로 DB receiving 검증 후 pl_db_comparison.xlsx 업데이트
...
PL_PATH      = '확인/PGU-DE-0565_BOP Piping_General System(확인).xlsx'
...
# 0565: PL도 FDR, DB도 FDR → 변환 불필요
print('0565 PL 파일 파싱...')
wb = openpyxl.load_workbook(PL_PATH, read_only=True)
ws = wb.active

# (pkg_no, norm_desc) 기준 수량 합산
...
print(f'  → {len(pl_grouped)}개 항목 (중복 합산 완료)')

# ── DB 0565 receiving 조회 ───────────────────────────────────────────
print('DB 조회...')
r = requests.get(
    f'{SUPABASE_URL}/rest/v1/receiving?select=id,pkg_no,mat_code,qty,unit,full_description'
    f'&doc_no=eq.PGU-DE-0565&order=pkg_no,id',
    headers=HEADERS
)
db_rows = r.json()
...
for (pkg_db, pl_desc_n), pl_info in sorted(pl_grouped.items()):
    ...
    pl_qty_db = pl_qty * 6.0 if pipe else pl_qty
    ...
    # 단어 유사도 fallback (같은 pkg_no 내에서만)
...
for i in range(2, ws_cmp.max_row + 1):
    ...
    if val and '0565' in str(val):
        rows_to_delete.append(i)
for i in reversed(rows_to_delete):
    ws_cmp.delete_rows(i)
print(f'  기존 0565 행 {len(rows_to_delete)}행 삭제')
...
print(f'  새 0565 결과 {len(results)}행 추가')
